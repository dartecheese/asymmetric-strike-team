#!/usr/bin/env python3
"""
Simple X.com Integrated Crypto Trading Bot
A working version that combines X.com analysis with your trading system.
"""
...
class SimpleXPost:
    def __init__(self, content: str, author: str, verified: bool, followers: int, 
        ...
        self.sentiment = random.uniform(-0.5, 1.0)  # Simulated sentiment
...
class XTradingBot:
    ...
    async def run_continuous(self, interval_minutes: int = 5):
        """Run continuous trading (demo mode with short interval)"""
        print(f"\n🔄 Starting continuous trading ({interval_minutes} min intervals)")
        ...
        try:
            while True:
                ...
                # Wait for next cycle
                if cycle < 3:  # Demo: run 3 cycles
                    print(f"\n⏳ Waiting {interval_minutes} minutes for next cycle...")
                    await asyncio.sleep(interval_minutes * 60)
                    cycle += 1
                else:
                    print("\n✅ Demo completed - 3 cycles run")
                    break
            ...
            self._save_trades()
            print(f"\n💾 Saved {len(self.trades)} trades to file")
    
    def _save_trades(self):
        """Save trades to JSON file"""
        try:
            filename = f"x_trades_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.json"
...
async def main():
    ...
    print("2. Continuous trading (3 cycles, 5 min intervals)")
    ...
    try:
        ...
        if choice == "1":
            ...
            await bot.run_continuous(interval_minutes=1)  # 1 min for demo
        else:
